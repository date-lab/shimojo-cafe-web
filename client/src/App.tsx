...
const AboutShimojocafe = lazy(() =>
  import("./pages/AboutShimojocafe").then((m) => ({ default: m.AboutShimojocafe }))
);
...
function AppRoutes() {
  ...
  const [shopLanguage, setShopLanguage] = useState<ShopLanguage>("ja");
  const resetShopLanguage = useCallback(() => {
    setShopLanguage("ja");
  }, []);
  const resetAll = useCallback(() => {
    ...
    resetCheckout();
    resetShopLanguage();
  }, [clearCart, resetCheckout, resetShopLanguage]);
    ...
    <Suspense fallback={<div className="page">読み込み中...</div>}>
      <Routes>
        <Route
          ...
          element={
            <Shop
              onIdleReset={resetAll}
              language={shopLanguage}
              onLanguageChange={setShopLanguage}
              onPurchaseLanguageReset={resetShopLanguage}
            />
          }
        />
        ...
        <Route path="/about" element={<AboutShimojocafe />} />
